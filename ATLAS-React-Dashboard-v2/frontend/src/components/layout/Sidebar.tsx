...
const items = [
  ...
  { to: "/statistics", label: "Research Statistics", icon: Sigma },
...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const visibleItems = settings.data?.developer_mode
    ? [...items, { to: "/developer", label: "Developer", icon: TerminalSquare }]
    : items;
    ...
    <>
      ...
      <aside className={["fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-outline-variant bg-surface px-4 py-8 transition-transform dark:border-slate-700 dark:bg-slate-950", open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"].join(" ")}>
        <div className="mb-8 flex items-start justify-between">
          <div><h1 className="text-2xl font-bold tracking-tight text-primary dark:text-white">ATLAS</h1><p className="mt-2 text-xs leading-5 text-text-muted dark:text-slate-400">Trastuzumab resistance computational pipeline</p></div>
        ...
        <NavLink to="/settings" onClick={onClose} className="mt-3 flex items-center gap-3 border-t border-outline-variant px-3 pt-4 text-sm text-text-muted dark:border-slate-700 dark:text-slate-300"><Settings className="h-4 w-4" />Settings</NavLink>
      </aside>
    </>
  );
}
